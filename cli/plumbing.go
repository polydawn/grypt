...
func PlumbingSmudge(ctx grypt.Context, keyring string, in io.Reader, out io.Writer) {
	keyDir := filepath.Join(ctx.RepoDataDir, "grypt")
	k, err := grypt.ReadKey(filepath.Join(keyDir, keyring+".key"))
	if err != nil {
...
func PlumbingTextconv(ctx grypt.Context, keyring string, f string, out io.Writer) {
	keyDir := filepath.Join(ctx.RepoDataDir, "grypt")
	k, err := grypt.ReadKey(filepath.Join(keyDir, keyring+".key"))
	if err != nil {
		panic(fmt.Errorf("error reading key: %v", err))
